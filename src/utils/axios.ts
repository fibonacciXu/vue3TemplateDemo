...
const errorHandle = (status: number, error): void => {
    ...
    switch (status) {
        ...
        case 500:
            return alert(
                `Error Code: ${status}, Message: ${
                    error.msg || "后台错误，请联系管理员"
                }`
            )
        case 502:
            return alert(
                `Error Code: ${status}, Message: ${error.msg || "平台环境异常"}`
            )
        default:
            alert(
                `Error Code: ${status}, Message: ${
                    error.msg || "未知错误，请刷新重试"
                }`
            )
    }
}
...
): Promise<IResponseData> => instance.post(url, params, config)
